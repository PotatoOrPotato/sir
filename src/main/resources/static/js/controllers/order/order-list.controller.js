app.controller('OrderListController',['$scope','$state','$http','$stateParams','toaster',function ($scope,$state,$http,$stateParams,toaster) {
    /**搜索起始页*/
    $scope.now = 1;
    /**每页显示的条数*/
    $scope.page_size = 10;
    ...
    function loadItem(page_index) {
        ...
        let url = $scope.app.apiUrl + "/order/page/" + (page_index - 1) + "/" + $scope.page_size;
    ...
    $scope.remove = function (sn) {
        $http.delete($scope.app.apiUrl + '/order/' + sn).success(function (res) {
            if (res.status === 80200) {
