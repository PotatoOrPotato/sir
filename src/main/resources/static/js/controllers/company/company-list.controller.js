app.controller('CompanyListController', ['$scope', '$http', '$state', 'toaster', function ($scope, $http, $state, toaster) {
    /**搜索起始页*/
    $scope.now = 1;
    /**每页显示的条数*/
    $scope.page_size = 10;
    ...
    function loadCompany(page_index) {
        $scope.now = page_index;
        let url = $scope.app.apiUrl + "/company/page/" + (page_index - 1) + "/" + $scope.page_size;
    ...
    $scope.edit = function(sn){
        $state.go("app.company.edit",{sn: sn});
    }
    /**
     * 删除
     * @param sn 数据的唯一标识
    ...
    $scope.remove = function (sn) {
        $http.delete($scope.app.apiUrl + '/company/' + sn).success(function (res) {
            if (res.status === 80200) {
                toaster.pop("success", "删除数据成功");
                loadCompany($scope.now);
            }
        }).error(function (err) {
