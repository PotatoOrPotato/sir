app.controller('ProductListController',['$scope','$state','$http','$stateParams','toaster',function ($scope,$state,$http,$stateParams,toaster) {
    /**搜索起始页*/
    $scope.now = 1;
    /**每页显示的条数*/
    $scope.page_size = 10;
    ...
    function loadItem(page_index) {
        ...
        let url = $scope.app.apiUrl + "/product/page/" + (page_index - 1) + "/" + $scope.page_size;
    ...
    $scope.edit = function(sn){
        $state.go("app.product.edit",{sn: sn});
    }
    /**
     * 删除
     * @param sn 数据的唯一标识
    ...
    $scope.remove = function (sn) {
        $http.delete($scope.app.apiUrl + '/product/' + sn).success(function (res) {
            if (res.status === 80200) {
