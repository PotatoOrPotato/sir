app.controller('ProductEditController', ['$scope', '$http', '$state', '$stateParams', 'toaster', 'FileUploader', function ($scope, $http, $state, $stateParams, toaster, FileUploader) {
    ...
    function loadItem() {
        ...
        $http.get($scope.app.apiUrl + "/product/" + $scope.sn).success(function (res) {
            if (res.status == 80200) {
    ...
    function loadCategory() {
        $http.get($scope.app.apiUrl+'/product/category').success(function (res) {
            if(res.status==80200){
    ...
    $scope.save = function () {
        ...
        if ($scope.sn == null || $scope.sn == '') {
            $http.post($scope.app.apiUrl + "/product/create", data).success(function (res) {
                if (res.status == 80200) {
                    toaster.pop('success', '添加', '添加成功');
                    $state.go("app.evaluate.list");
                }
            }).error(function (err) {
                toaster.pop('error', '添加失败', JSON.stringify(err));
            })
        } else {
            $http.put($scope.app.apiUrl + "/product/" + $scope.sn, data).success(function (res) {
                if (res.status == 80200) {
